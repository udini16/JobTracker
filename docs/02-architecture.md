...
- **Role**: Exposes REST API endpoints, orchestrates web scraping via Puppeteer, interacts with the LLM API, sends emails via Nodemailer, and pushes notifications via Telegram.
...
3. User sends email -> Backend sends via Nodemailer with tracking pixel -> Returns success.
